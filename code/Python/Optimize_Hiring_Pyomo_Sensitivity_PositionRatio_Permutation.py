...
### Combination of head count ratio are generated as permutation of multiple parameters
...
ratio_range = np.arange(start=2.0, stop=6.0, step=0.5)
ratio_range = np.round(ratio_range, decimals=2).tolist()
...
ratio_hc_list = list(itertools.product(*ratio_range_list))
...
for ind in range(len(ratio_hc_list)):
    ...
    print(f"Completed: {ind}/{n}")

ratio_df.to_csv(out_dir + '/sensitive_analysis_hc_ratio_perm.csv', index = False)